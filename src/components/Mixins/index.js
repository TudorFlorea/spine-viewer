...
const Mixins = (props) => {
	const { animations, onAddMixin = () => {} } = props;
	...
	const handleAddMixin = () => {
		...
		const hasMixin =
			mixins.findIndex(
				(mixin) =>
					mixin.fromAnim === currentMixin.fromAnim &&
					mixin.toAnim === currentMixin.toAnim
			) !== -1;
		...
		return () => {
			...
			setMixins(
				mixins.filter(
					(mixin) =>
						!(
							mixin.fromAnim === mixinToDelete.fromAnim &&
							mixin.toAnim === mixinToDelete.toAnim
						)
				)
			);
			onAddMixin({
	...
	return (
		<Draggable cancel="mixin--duration">
			<div className="card mixins">
				{showMixins ? (
					<>
						<Button text="Hide mixins" onClick={onShowMixinsToggle} />
						<div className="row">
												...
												<tr key={mixin.id}>
													...
													<td>
														<a href="#!" className="secondary-content">
															<i
																onClick={handleMixinDelete(mixin)}
																className="material-icons red-text text-darken-1"
															>
																delete
															</i>
														</a>
					...
					<Button text="Show mixins" onClick={onShowMixinsToggle} />
				)}
			</div>
		</Draggable>
	);
