...
const SpineData = (props) => {
	...
	const { animations, skins, slots, bones } = spineData;
		...
		<Draggable cancel=".spine-elements">
			<div className="animations card">
				<div className="card-content">
					{showAnimations ? (
						<>
							<Button text="Hide spine data" onClick={onShowAnimations} />
										...
										{animations.map((anim) => {
											return (
												<SpineDataEntry
													key={anim}
													name={anim}
													onNameClick={handleAnimationClick(anim)}
												/>
											);
											// return <Button key={anim} onClick={handleAnimationClick(anim)} text={anim} className="animations--button" />
										})}
										<Button
											onClick={handleSetupPoseClick}
											text="Setup pose"
											className="animations--button lime darken-2"
										/>
									</div>
								</li>
								<li>
											...
											<>
												{skins.map((skin) => {
													return (
														<SpineDataEntry
															key={skin}
															onNameClick={handleSkinClick(skin)}
															name={skin}
														/>
													);
												})}
									...
									<div className="collapsible-body">
										{slots.length ? (
											<>
												{slots.map((slot) => {
													return (
														<SpineDataEntry key={slot} name={slot} disabled />
													);
												})}
											</>
										) : (
									...
									<div className="collapsible-body">
										{bones.length ? (
											<>
												{bones.map((bone) => {
													return (
														<SpineDataEntry key={bone} name={bone} disabled />
													);
												})}
											</>
										) : (
						...
						<Button text="Show spine data" onClick={onShowAnimations} />
					)}
				</div>
			</div>
		</Draggable>
